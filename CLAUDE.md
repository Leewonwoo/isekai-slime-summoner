...
십자형 타워 디펜스 × 로그라이크 × 키우기. **테마: 인간 소환사가 고블린 용병단(유닛 8종, 정면/후면/측면 3뷰 스프라이트)을 소환해 슬라임 군단(형태 4종 × 속성 틴트)을 막는다. 속성 상성 4종(무/화염/빙결/자연, 화>자연>빙>화) — SPEC §2.3/2.7/2.8.** 중앙 코어 + 4방향 레인(레인당 슬롯 3칸), 유닛 소환·3합성(오토체스식)·재배치. 웨이브 20개 + 보스. 웨이브는 **규칙 기반**(SO 웨이브 테이블 + 방향 가중치 + 랜덤 시드 변주) — **런타임 LLM API 호출은 스코프 아웃**, AI는 개발 도구(코드·에셋·밸런싱 CSV 생성)로만 사용.
